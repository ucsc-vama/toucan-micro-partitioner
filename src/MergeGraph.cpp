...
void MergeGraph::graph_gc() {
    for (int node : nodes_to_remove) {
        ...
        for (int pred : get_node_predecessors(node)) {
            ...
            successors.erase(std::remove(successors.begin(), successors.end(), node), successors.end());
        }
        
        for (int succ : get_node_successors(node)) {
            auto& predecessors = reverse_adjacency_list[succ];
            predecessors.erase(std::remove(predecessors.begin(), predecessors.end(), node), predecessors.end());
        }
...
void MergeGraph::remove_edge(int from, int to) {
    ...
    predecessors.erase(std::remove(predecessors.begin(), predecessors.end(), from), predecessors.end());
}
