...
std::unordered_map<int, std::vector<int>> load_vec_info_file(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (ret.find(vecDecl_node_id) != ret.end()) {
            throw std::runtime_error("Duplicate vecDecl_node_id: " + std::to_string(vecDecl_node_id));
        }
...
int min_value(const std::vector<int>& values) {
    if (values.empty()) throw std::runtime_error("Cannot find min of empty vector");
    return *std::min_element(values.begin(), values.end());
}

int max_value(const std::vector<int>& values) {
    if (values.empty()) throw std::runtime_error("Cannot find max of empty vector");
