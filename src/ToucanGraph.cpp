...
NodeTag string_to_node_tag(const std::string &str) {
    if (str == "ConstDecl")
        return NodeTag::ConstDecl;
    if (str == "RegRead")
        return NodeTag::RegRead;
    if (str == "MemRead")
        return NodeTag::MemRead;
    if (str == "VecDecl")
        return NodeTag::VecDecl;
    if (str == "VecDecl_LUT_NOP")
        return NodeTag::VecDecl_LUT_NOP;
    if (str == "VecRead")
        return NodeTag::VecRead;
    if (str == "LUT")
        return NodeTag::LUT;
    if (str == "VecArith")
        return NodeTag::VecArith;
    if (str == "VecLogic")
        return NodeTag::VecLogic;
    if (str == "Print")
        return NodeTag::Print;
    if (str == "Stop")
        return NodeTag::Stop;
    if (str == "RegWrite")
        return NodeTag::RegWrite;
    if (str == "MemWrite")
        return NodeTag::MemWrite;
...
const char *node_tag_to_string(NodeTag tag) {
    ...
    case NodeTag::VecDecl:
        return "VecDecl";
    case NodeTag::VecDecl_LUT_NOP:
        return "VecDecl_LUT_NOP";
...
bool is_valid_node_tag(NodeTag tag) { return tag != NodeTag::UNKNOWN; }
...
void ToucanGraph::load(const std::string &file_path) {
    ...
    while (std::getline(file, line)) {
        ...
        if (parts.size() < 3) {
            throw std::runtime_error("Node " + std::to_string(node_id) +
                                     " is missing weight or neighbors.");
        }

        std::string label = parts[1];
        if (label.empty()) {
            throw std::runtime_error("Node " + std::to_string(node_id) +
                                     " has an empty label, which is illegal.");
        }
        ...
        if (tag == NodeTag::UNKNOWN) {
            throw std::runtime_error("Unknown node tag/label '" + label + "' at node " +
    ...
    for (const auto &edge : edges_to_add) {
        ...
        if (!has_node(source) || !has_node(target)) {
            throw std::runtime_error("Edge from " + std::to_string(source) + " to " +
                                     std::to_string(target) + " refers to non-existent node.");
        }
    ...
    if (nodes.size() != static_cast<size_t>(num_nodes)) {
        throw std::runtime_error("Expected " + std::to_string(num_nodes) + " nodes, found " +
                                 std::to_string(nodes.size()));
    }
    if (actual_edge_count != num_edges) {
        throw std::runtime_error("Expected " + std::to_string(num_edges) + " edges, found " +
                                 std::to_string(actual_edge_count));
    }
}
...
void ToucanGraph::expand_VecDecl(
    ...
    std::cout << "Expand " << nodes_to_remove.size() << " vectors, add " << num_new_nodes
              << " new VecDecl_LUT_NOP nodes\n";
}
